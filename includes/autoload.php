...
class ClassLoader
{
    ...
    /**
     * List all classes of the given namespace.
     *
     * @param string $namespace The namespace
     * @param bool $recursive Search classes recursively (Default: FALSE)
     * @return array|bool Returns an array of classes in the namespace. If the namespace is not found FALSE is returned.
     */
    public function listClasses($namespace, $recursive = false)
    {
        ...
        list($path_prefix, $path_suffix) = explode('*', $pattern);

        $namespace_without_prefix = substr($namespace, strlen($prefix));
        $path = $path_prefix . str_replace('\\', '/', $namespace_without_prefix);

        $start = strlen($path_prefix);
        $end = strlen($path_suffix) * -1;
        ...
        if ($recursive) {
            ...
            foreach ($iterator as $file) {
                if (substr($file->getBasename(), $end) == $path_suffix) {
                    $classes[] = str_replace('/', '\\', substr($file->getPathName(), $start, $end));
                }
            }
        } else {
            $iterator = new \DirectoryIterator($path);
            foreach ($iterator as $file) {
                if ($file->isFile() && substr($file->getBasename(), $end) == $path_suffix) {
                    $classes[] = substr($file->getBasename(), 0, $end);
                }
            }
        }
